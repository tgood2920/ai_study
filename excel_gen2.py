def write_sheet2(workbook, data, title_fmt, head_fmt, cell_fmt):
    ...
    for col, h in enumerate(headers): ws.write(curr_row, col, h, head_fmt)
    curr_row += 1
    for idx, doc in enumerate(data.get('prep_docs', []), start=1):
        ...
        curr_row += 1
    ws.set_column(1, 1, 45); ws.set_column(2, 4, 15)